...
for root, sub, files in os.walk(r_dir):
    ...
    for f in files:       
        ...
        for delta_t in intervalos:
            ...
            pp.plot(time_part,filtered_aver_vs_ped_simp, linestyle='--',color='b',label='Low-pass Filter envelope',linewidth=3)

            # Rms envelope #
            pp.plot(time_rms+500/float(w[0]),x1_part_rms, linestyle='-',color='k',label='RMS aproach envelope',linewidth=1)
            ...
            pp.xticks(np.arange(delta_t*float(1)/float(w[0]),(delta_t+step)*float(1)/float(w[0])+0.001,0.1),fontsize = 12)
            ...
            pp.specgram(x1, NFFT=int(w[0]*0.01) ,  window= scipy.signal.tukey(int(w[0]*0.01)), Fs=w[0], noverlap = int(w[0]*0.005),cmap='jet')               
            ...
            figname = "%s.jpg" %(str(base)+'_signal_zoom_'+str(delta_t*float(1)/float(w[0])))        
